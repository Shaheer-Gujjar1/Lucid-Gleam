...
export function MarksSheet({ classId }: MarksSheetProps) {
  ...
  const calculateAutoMarks = useCallback((studentId: string, column: MarksColumn): number => {
    ...
    switch (column.autoSource) {
      ...
      case "assignments": {
        const assignmentTasks = tasks.filter(t => t.type === "assignment" && t.includeInMarksSheet);
      ...
      case "quizzes": {
        const quizTasks = tasks.filter(t => t.type === "quiz" && t.includeInMarksSheet);
      ...
      case "presentations": {
        const presTasks = tasks.filter(t => t.type === "presentation" && t.includeInMarksSheet);
      ...
      case "projects": {
        const projectTasks = tasks.filter(t => t.type === "project" && t.includeInMarksSheet);
